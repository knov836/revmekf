...
class PredictFilter(Filter):
    ...
    def linalg_correct(self,Accelerometer,Pressure,Surface,Orient):
        ...
        acc1,rot1,irot1,acc2,rot2,irot2,acc3,rot3,irot3= acc_from_normal1(mm1,mm1 , np.array(quat_rot(quat_rot([0,*(Accelerometer*(self.dt**2))],quat_inv(qq)),quat0))[1:4], np.array(quat_rot([0,*np.array([0,0,1])],quat0))[1:4], np.array(quat_rot([0,*self.surf_center],quat0))[1:4],start = logrot1,s_rot=self.state[3:6])
