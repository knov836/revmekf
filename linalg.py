...
def acc_from_normal_imu_grav_neural(norm0,norm,acc,grav,normal,center,start=[0,0,1],s_rot=np.array([0,0,0]),heuristic=False,correction=False,speed=np.array([0,0,0]),magneto =np.array([0,1,0])):
    ...
    if heuristic:
        ...
        t_v0_acc = FF_normal[1]
        #print("ff normal",FF_normal)
        if np.abs(t_v0_acc-t_v0)>6*np.pi/4:
            #pdb.set_trace()
...
def acc_from_normal_imu_grav_manual(norm0,norm,acc,grav,normal,center,start=[0,0,1],s_rot=np.array([0,0,0]),heuristic=False,correction=False,ind=0,speed=np.array([0,0,0]),magneto =np.array([0,1,0])):
    ...
    if heuristic:
        ...
        if (corrected):
            ...
            t1t0 = t4t0#np.sign(float(t4t0-t_v0))*np.abs(float(np.abs(t4t0-t_v0)))/2+t_v0
        elif (t4t0 == t2t0 and sign*(teGG-C).dot(normal).evalf(subs={v:(t4t0+t2t0)/2})<0) or sign*(teGG-C).dot(normal).evalf(subs={v:(t4t0+t2t0)/2})<0  or np.abs(t_v0-t3t0)>np.abs(t4t0 -t2t0)*1.5:
            qq0 = ExpQua(t_v0*np.array(FF).flatten())
            pracc = np.copy(np.array(quat_rot([0,*(acc/np.linalg.norm(acc))],qq0))[1:4]).astype(float)
            pracc = pracc/np.linalg.norm(pracc)
            qq1 = quat_ntom(pracc, np.array([0,0,1]))
...
def acc_from_normal_imu_grav(norm0,norm,acc,grav,normal,center,start=[0,0,1],s_rot=np.array([0,0,0]),heuristic=False,correction=False):
    ...
    """l_HH = (teGG-C).dot(normal).subs(v,v+sym.Rational(float(t_v0)))
    plot(l_HH)
    print(np.linalg.norm(acc),np.linalg.norm(center))
    print(t_v0,t1t0,t2t0)
    if t1t0 != t2t0:
        x_pts = [float(t4t0 - t_v0), float(t2t0 - t_v0)]
        y_pts = np.array([l_HH.evalf(subs={v: float(t4t0 - t_v0)}),
         l_HH.evalf(subs={v: float(t2t0 - t_v0)})]).astype(float)
        labels = [r'$\Theta_1$',r'$\Theta_2$']
        tt = np.max([np.abs(float(t4t0)),np.abs(float(t2t0))])
        tt = np.pi*1/4
        f = sym.lambdify(v, l_HH, 'numpy')
        vv = np.linspace(-tt, tt, 1000)
        fig, ax = plt.subplots(figsize=(6,4))
        ax.plot(vv, f(vv), label=r'$h_k(\Theta)$', color='blue')
        ax.scatter(x_pts, y_pts, color='red', marker='o', s=60,label='Intersection points')
    ...
    if heuristic:
        ...
        if (t4t0 == t2t0 and sign*(teGG-C).dot(normal).evalf(subs={v:(t4t0+t2t0)/2})<0) or sign*(teGG-C).dot(normal).evalf(subs={v:(t4t0+t2t0)/2})<0:#  or np.abs(t_v0-t3t0)>np.abs(t4t0 -t2t0)*1.5:
            ...
            y_pts = np.array([l_HH.evalf(subs={v: float(t1t0 - t_v0)}),
             l_HH.evalf(subs={v: float(t4t0 - t_v0)}),
             l_HH.evalf(subs={v: float(t2t0 - t_v0)})]).astype(float)
            labels = ['MEKF',r'$\Theta_1$',r'$\Theta_2$']
            tt = np.max([np.abs(float(t4t0-t_v0)),np.abs(float(t2t0-t_v0))])
            f = sym.lambdify(v, l_HH, 'numpy')
            vv = np.linspace(-tt, tt, 1000)
            fig, ax = plt.subplots(figsize=(6,4))
