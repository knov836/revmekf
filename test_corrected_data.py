...
p_start = p0-10
p_end = p0+10
for p0 in np.argwhere(correction_applied).flatten()[2:]:
    p_start = p0-50
    p_end = p0+50
    fig = plt.figure()
    ax = fig.add_axes([0,0,1,1])
    ...
    fig = plt.figure()
    ax = fig.add_axes([0,0,1,1])
    ax.plot(time0[p_start:p_end],[np.linalg.norm(newset.acc[j,:2]) for j in range(p_start,p_end)])
    ax.plot(time0[p0], [np.linalg.norm(newset.acc[j,:2]) for j in [p0]],'.',**dict(markersize=10))
    ...
    fig = plt.figure()
    ax = fig.add_axes([0,0,1,1])
    ax.plot(time0[p_start:p_end],[normals[j,:] for j in range(p_start,p_end)])
    ax.plot(time0[p0], [normals[j,:] for j in [p0]],'.',**dict(markersize=10))
    
    ax.legend(['Normals','Corrections applied by Rev-MEKF'])
    ax.set_title('Corrected places on normals')
    plt.show()
