...
class SolverFilterPlan:
    def __init__(self,kfilter,q0,q1,r0,r1,normal,newset,start=np.array([1,0,0,0],dtype=mpf),gravity=np.array([0,0,1],dtype=mpf),proj_fun=None,size=100,heuristic=False,neural=False,manual=False):
        """
        Parameters
        ----------
        kfilter : TYPE
    ...
    def update(self,time,*args,**kargs):#gyro,acc,mag,normal):
        ...
        if newset==None:
            ...
            if self.mode == 'GyroAccMag':
                ...
                if "xtensor" in kargs.keys():
                    xtensor = kargs["xtensor"]
